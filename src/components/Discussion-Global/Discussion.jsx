...
const Discussion = ({ comment }) => {
	...
	const addReply = (commentId) => {
		if (myReply.length === 0)
			return toast.warning("Balasan kamu masih kosong!");
					...
					<li className="comment-item">
						<div className="d-flex">
							<div className="avatar avatar-sm flex-shrink-0">
								<img
									...
									src={
										comment?.User?.display_picture_link == undefined
											? user.display_picture_link
											: comment?.User?.display_picture_link == null
											? "/assets/images/avatar/empty-display-picture.png"
											: comment?.User?.display_picture_link
									}
						...
						<div className="d-flex mt-3 justify-content-between">
							<button
								...
								onClick={() => doLikeComment(comment.id)}
							>
							...
							>
								...
								></i>{" "}
								{showCommentBox ? "Tutup Balasan" : "Tambah Balasan"}
							</button>
						</div>
											...
											<div className="d-flex">
												<div className="avatar avatar-sm flex-shrink-0">
													<img
														className="avatar-img rounded-circle"
														src={
															reply?.User?.display_picture_link == undefined
																? user.display_picture_link
																: reply?.User?.display_picture_link == null
																? "/assets/images/avatar/empty-display-picture.png"
																: reply?.User?.display_picture_link
														}
														// alt={
														// 	reply?.User?.full_name != undefined
												...
												<div className="ms-2 w-100">
													<h6 className="mb-0">{reply.User.full_name}</h6>
													<small>
															...
															<ul className="dropdown-menu">
																<li>
																	<button
																		className="dropdown-item"
																		type="submit"
																		onClick={() => doDeleteReply(reply.id)}
																	>
																		Hapus Balasan
																	</button>
																</li>
