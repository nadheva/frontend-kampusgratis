...
import { getAllDiscussion, likeDiscussion, resetAll, deleteDiscussion } from '../../features/discussion-global/discussionGlobalSlice';
...
const MainContent = () => {
  ...
  const doShare = async (discussionTitle, discussionsId) => {
    await copyTextToClipboard(`Diskusi ${discussionTitle}. URL Halaman: ${window.location}/diskusi/${discussionsId}`);
  }
      ...
      <section className="pb-4">
        <div className="row position-relative">
          ...
          <div className="col-lg-2 p-2">
            <Link className='btn btn-primary' to="/diskusi-global/buat-diskusi">Buat Diskusi Baru</Link>
          </div>
        </div>
      </section>
              ...
              <div className="list-unstyled mb-0">
                <div className="comment-item">
                  <div className="d-flex mb-3">
                    <div className="avatar avatar-sm flex-shrink-0">
                      <img
                        ...
                        src={discussion?.User?.display_picture_link == undefined ? user.display_picture_link :
                          discussion?.User?.display_picture_link == null ? '/assets/images/avatar/empty-display-picture.png' : discussion?.User?.display_picture_link}
                        alt={discussion?.User?.full_name != undefined ? discussion?.User?.full_name : user?.full_name}
                      />
                    </div>
                    <div className="ms-2 w-100">
                      <h6 className="mb-0">
                        {discussion?.User?.full_name == undefined ? user.full_name : discussion?.User?.full_name}
                      </h6>
                      <small>
                    ...
                    {discussion?.User?.display_picture_link === user.display_picture_link && <>
                      <div className="dropdown">
                        <span className="btn btn-xs btn-round mx-1" type="button" data-bs-toggle="dropdown" aria-expanded="true">
                          X
                        </span>
                        <ul className="dropdown-menu">
                          <li><button className="dropdown-item" type="submit" onClick={() => doDeleteDiscussion(discussion.id)}>Hapus Diskusi</button></li>
                        </ul>
                      </div>
                    </>}
                  </div>
                  ...
                  <div className="d-flex mt-3 justify-content-between">
                    ...
                    <button className='btn btn-sm w-50 py-2 border ms-2' onClick={() => doShare(discussion.title, discussion.id)}>
