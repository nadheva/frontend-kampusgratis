...
const Form = () => {
	...
	const educationList = [
		{ value: "", label: "-" },
		{ value: "Tamat SD", label: "Tamat SD / Sederajat" },
		{ value: "SLTP", label: "Tamat SMP / SLTP / Sederajat" },
		{ value: "SLTA", label: "Tamat SMA / SMK / SLTA / Sederajat" },
		{ value: "Lainnya", label: "Lainnya" },
	];
	...
	const incomeList = [
		{ value: "", label: "-" },
		{
			value: "Rp1.000.000 s/d Rp2.000.000",
			label: "Rp1.000.000 s/d Rp2.000.000",
		},
		{
			value: "Rp2.000.001 s/d Rp3.000.000",
			label: "Rp2.000.001 s/d Rp3.000.000",
		},
		{
			value: "Rp3.000.001 s/d Rp4.000.000",
			label: "Rp3.000.001 s/d Rp4.000.000",
		},
		{
			value: "Rp4.000.001 s/d Rp5.000.000",
			label: "Rp4.000.001 s/d Rp5.000.000",
		},
	];

	const [isPageLoaded, setIsPageLoaded] = useState(false);
	const [isAdministrationAccepted, setisAdministrationAccepted] =
		useState(false);
	...
	const onChangeFile = (e) => {
		if (data?.is_approved?.overall)
			return toast.warning(
				"Kamu tidak dapat merubah data administrasi karena administrasimu sudah dikonfirmasi!"
			);
	...
	const onFormSubmitBiodata = (e) => {
		...
		if (
			Object.values(administrationBiodata).some((el, index) => {
				let index_optional = [
					"study_program",
					"semester",
					"nsn",
		...
		console.log(administrationBiodata);
	...
	useEffect(() => {
		...
		if (message && isError) {
			toast.error(
				"Masih ada field yang kosong atau tidak valid. Mohon isi semua field!"
			);
			dispatch(reset());
		}
		...
		if (Object.values(data).length !== 0) {
			...
			let appendData = {};
			if (data?.biodata?.full_name === null)
				appendData.full_name = user.full_name;
			...
			if (
				data?.biodata &&
				Object.values(administrationBiodata).some((el) => el === "")
			)
			...
			>
				{!isPageLoaded && isLoading ? (
										...
										<div className="row">
											<div className="col-md-6">
												...
												<input
													...
													readOnly={isLoading}
													disabled={isLoading || isAdministrationAccepted}
												/>
											</div>
											<div className="col-md-6">
												<label className="form-label">
                    ...
                    <input type="text" className="form-control mb-3" name="last_education" value={last_education} placeholder="Pendidikan Terakhir" onChange={onFormChangeAdministrationBiodata} readOnly={isLoading} disabled={isLoading || isAdministrationAccepted} /> */}
											...
											<div className="col-md-6">
												<label className="form-label">
													<span className="text-danger"></span> NIM atau NPM
												</label>
												<sub>&nbsp;(opsional)</sub>
												<input
													type="text"
											...
											<div className="col-md-6">
												<label className="form-label">
													<span className="text-danger"></span> Universitas Asal
												</label>
												<sub>&nbsp;(opsional)</sub>
												<input
													type="text"
											...
											<div className="col-md-6">
												<label className="form-label">
													<span className="text-danger"></span> Program Studi
												</label>
												<sub>&nbsp;(opsional)</sub>
												<input
													type="text"
								...
								<div className="accordion-body mt-3">
									<form onSubmit={onFormSubmitFiles}>
										<div className="row">
											...
											<div className="col-12">
												<label className="form-label">
													<span className="text-danger">*</span> Ijazah Terakhir
												</label>
												{certificate?.name && (
													<div className="bg-light mb-3 p-3">
														{certificate?.name}
