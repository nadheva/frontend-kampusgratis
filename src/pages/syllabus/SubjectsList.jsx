...
const SubjectsList = () => {
	...
	const { isLoading, data, message } = useSelector((state) => state.syllabus);
	...
	useEffect(() => {
		...
		if (message.startsWith("Enrolled to") && isTakingMajor) {
			...
			toast.info(
				`Kamu telah mengambil Program Studi ${
					message.split("Enrolled to ")[1]
				}.`
			);
			dispatch(reset());
		...
		if (message === "Student's major doesn't have that subject") {
			dispatch(reset());
			toast.error(
				"Kamu tidak dapat mengambil Mata Kuliah ini karena berbeda dengan Rencana Studi kamu."
			);
		}
		...
		if (data?.study_plan?.students_information?.Majors) {
			...
			let eligible = data?.study_plan?.students_information?.Majors.find(
				(major) => major.StudentMajor.major_id === data?.subjects?.major?.id
			);
										...
										<div className="d-flex justify-content-center">
											<nav aria-label="breadcrumb">
												<ol className="breadcrumb breadcrumb-dark breadcrumb-dots mb-0">
													...
													<li className="breadcrumb-item">
														<Link to="/silabus">Rencana Studi</Link>
													</li>
													<li
									...
									{user?.role === "guest" && (
										<>
											<div className="alert alert-info">
												Haaaii {user?.full_name.split(" ")[0]}! Kamu harus
												mengirimkan data{" "}
												<Link to="/administrasi">administrasi</Link> sebelum
												mengambil Rencana Studi atau mata kuliah.
											</div>
										</>
									)}
									{user?.role === "student" && (
											...
											<div className="col-lg-12 mt-4 order-1">
														...
														{userStudyPlan?.total_credit === 24 ? (
															<>
																<div className="alert alert-warning">
																	Kamu telah melampaui batas maksimal
																	pengambilan SKS (24 SKS). Kamu tidak dapat
																	mengambil SKS kembali.
																</div>
															</>
														) : (
															<>
																<div className="alert alert-success">
																	Kamu dapat mengambil Mata Kuliah pada Rencana
																	Studi ini.
																</div>
															</>
														)}
												...
												{!eligibleTakeMajor &&
													...
													userMajors.length > 0 && (
														<>
															<div className="alert alert-warning">
																Kamu telah mengambil Rencana Studi{" "}
																<strong>{userMajors[0].name}</strong>. Kamu
																tidak dapat mengambil Rencana Studi atau mata
																kuliah lainnya!
															</div>
														</>
													)}
