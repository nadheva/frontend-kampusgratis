...
const QuizDetail = () => {
  ...
  useEffect(() => {
    if (userAnswersTemp && Object.keys(userAnswers).length === 0) {
      let userAnswersTempDecode = JSON.parse(userAnswersTemp);
      userAnswersTempDecode[0].answers.map((answer, number) => {
        setUserAnswers((prevState) => ({
          ...prevState,
    ...
    if (data?.questions && Object.keys(currentQuizQuestions).length === 0) {
      ...
      if (!userAnswersTemp) data.questions.quiz.questions.map((_, number) => setUserAnswers((prevState) => ({
        ...prevState,
        [`answer-${number + 1}`]: null
      })));
    }
  }, [data, currentQuizNumber, userAnswersTemp, userAnswers]);
                        ...
                        <div className="px-4 mx-3 py-4">
                          ...
                          <div className="btn-group d-block">
                            {currentQuizQuestions.map((_, number) => <>
                              <button className={`btn ${currentQuizNumber === number + 1 ? 'btn-primary' : 'btn-outline-primary'} me-1`} onClick={() => setCurrentQuizNumber(number + 1)}>{number + 1}</button>
                            </>)}
                          </div>
                        </div>
                      </div>
