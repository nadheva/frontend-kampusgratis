...
const QuizDetail = () => {
	...
	const onUserDoubt = () => {
		setUserAnswersDoubt(prevState => ({
			...prevState,
			[`answer-${currentQuizNumber}`]: !Object.values(userAnswersDoubt)[currentQuizNumber - 1]
		}))
	}
	...
	const doSubmitQuiz = () => {
		...
		if (answers.length !== currentQuiz.length) return toast.error("Kamu belum menjawab semua soal yang tersedia.");
	...
	}, [data, currentQuizNumber, userAnswersTemp, userAnswers, userAnswersDoubt, location]);
												...
												<div className='bs-stepper stepper-outline'>
													<div className="card-body">
														<div className="bs-stepper-content pt-1">
															<form>
																{currentQuizQuestions.map((question, number) => <>
																	<div className={`content dstepper-block ${currentQuizNumber === number + 1 ? 'active' : 'fade dstepper-none'}`}>
																		...
																		<div className="vstack gap-2">
																			{question.choices.map((choice, choiceNumber) => <>
																				<input type="radio" className="btn-check" name={`answer-${number + 1}`} id={`option${number}${(choiceNumber + 1) * choiceNumber}`} value={choice} onChange={onUserAnswer} checked={userAnswers[`answer-${number + 1}`] === choice} />
																		...
																		<div className="d-flex justify-content-center mt-3">
																			{currentQuizNumber !== 1 && <>
																				<button type="button" className="btn btn-primary mb-0" onClick={() => setCurrentQuizNumber(currentQuizNumber - 1)}>
																			...
																			{!Object.values(userAnswersDoubt)[currentQuizNumber - 1] ? <>
																				<button type="button" className="btn btn-warning mb-0" onClick={() => onUserDoubt()}>
																					<i className="fa fa-solid fa-question"></i> <span className='d-md-inline d-none ms-2'>Ragu</span>
																				</button>
																			</> : <>
																				<button type="button" className="btn btn-secondary mb-0" onClick={() => onUserDoubt()}>
																			...
																			{currentQuizNumber !== currentQuizQuestions.length && <>
																				<button type="button" className="btn btn-primary mb-0" onClick={() => setCurrentQuizNumber(currentQuizNumber + 1)}>
																					<i className="fa fa-arrow-right"></i> <span className='d-md-inline d-none ms-2'>Pertanyaan Selanjutnya</span>
																				</button>
																			</>}
																			&nbsp;
																			{currentQuizNumber === currentQuizQuestions.length && <>
											...
											<div className="card bg-transparent border rounded-3 mb-1">
												...
												<div className="px-4 mx-3 py-4">
													...
													<div className="btn-group d-block">
														{currentQuizQuestions.map((_, number) => <>
															<button className={
																`btn ${Object.values(userAnswersDoubt)[number] ? 'btn-warning'
																	: currentQuizNumber === number + 1 ? 'btn-primary'
																		: Object.values(userAnswers)[number] ? "btn-success"
																			: 'btn-outline-primary'} me-1`} onClick={() => setCurrentQuizNumber(number + 1)}>{number + 1}</button>
														</>)}
													</div>
												</div>
											</div>
