...
const QuizStart = () => {
	...
	const { isLoading, data, message } = useSelector((state) => state.myStudy);
	...
	useEffect(() => {
		if (message === "SUCCESS_TAKE_QUIZ")
			navigate(`${data?.quiz_detail?.material_enrolled_id}`);
		...
		if (
			Object.keys(currentQuizzes).length !== 0 &&
			Object.keys(currentQuiz).length !== 0
		) {
			let currentindex = currentQuizzes.findIndex(
			...
			if (!isQuizSubmit && currentQuizIndex >= 0) {
				if (
					new Date(currentQuizzes[currentQuizIndex].deadline).getTime() <
					new Date().getTime()
				)
					doSubmitQuiz();
			}
												...
												<>
													<div className="col-md-12 mb-4">
														<div className="alert alert-info mb-4">
															...
															Kamu telah memulai Quiz ini. Kamu harus
															menyelesaikan pada{" "}
															<b>
																{
																	new Date(
																		currentQuizzes[currentQuizIndex].deadline
																	)
																		.toISOString()
																		.split("T")[0]
																}
														...
														<div className="d-sm-flex justify-content-sm-between mb-2 mb-sm-3">
															<div>
																<h5 className="card-title mb-0">
																	<Link
																		to={`/studi-ku/${subjectId}/pertemuan/${sessionId}/modul/${module.id}`}
																	>
														...
														<div className="d-sm-flex align-items-center">
															<Link
																...
																to={`${currentQuizzes[currentQuizIndex].material_enrolled_id}`}
															>
												...
												<>
													<div className="col-lg-12 col-xl-12">
														{currentQuiz.summary.length !== 3 ? (
															<>
																<div className="alert alert-info mb-4">
																	Kamu dapat memulai Quiz ini dengan klik tombol
																	`Mulai Quiz` di bawah ini.
																</div>
																<div className="d-sm-flex justify-content-sm-between mb-2 mb-sm-3">
																	<div>
																		<h5 className="card-title mb-0">
															...
															<>
																<div className="alert alert-info mb-0">
																	Kamu sudah mengambil Quiz sebanyak 3 kali dan
																	kamu sudah tidak dapat mengambil Quiz ini
																	lagi.
																</div>
															</>
														)}
																		...
																		<div className="card shadow border p-2">
																			<div className="card-body">
																				<h4 className="card-title">
																					Quiz Pengambilan Ke-{number + 1}
																				</h4>
																				<span className="badge bg-dark">
																					Pengumpulan kamu :{" "}
																					{
																						quizTaken.activity_detail
																							.date_submit
																					}
																				</span>
																				...
																				{quizTaken.status === "FAILED" ? (
																					...
																					<>
																						<span className="badge bg-sucess">
																							{quizTaken.status}
																						</span>
																					</>
																				)}
															...
															currentQuizzes.length !== 0 ? (
																<>
																	<div className="alert alert-danger m-3 mb-0">
																		Kamu sedang memulai Quiz. Kamu tidak dapat
																		melihat riwayat Quiz kamu!
																	</div>
																</>
															) : (
																<>
																	<div className="alert alert-info m-3 mb-0">
																		Belum ada Quiz yang telah kamu selesaikan
																		saat ini.
																	</div>
																</>
															)}
